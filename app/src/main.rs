...
impl LtdcDisplay {
    ...
    async fn swap(&mut self) -> Result<(), ltdc::Error> {
        ...
        self.ltdc.set_buffer(self.layer_config.layer, buf as *const _).await
    }
...
impl Button {
    fn new(x: i32, y: i32, width: i32, height: i32, label: &'static str, color: Rgb888) -> Self {
        Self { x, y, width, height, label, color, pressed: false }
    }
...
async fn main(spawner: Spawner) {
    ...
    loop {
        ...
        Text::with_baseline("STM32H7B3 Touch UI", Point::new(80, 30), title_style, Baseline::Alphabetic)
            .draw(display).unwrap();
        ...
        Text::with_baseline(counter_text.as_str(), Point::new(150, 200), counter_style, Baseline::Alphabetic)
            .draw(display).unwrap();
